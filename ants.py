...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        super().reduce_health(reflective_damage)

        bees_list = place_holder.bees[:]

        for bee in bees_list:
            if self.health <= 0:
                bee.reduce_health(self.damage +reflective_damage)
            else: 
...
class WallAnt(Ant):
    ...
    def __init__(self, health=4): ## creates ant with health of 4
        super().__init__(health)
...
class HungryAnt(Ant):
    ...
    def __init__(self,health=1,turns_to_chew=0): ## creates ant with health of 1
    ...
    def action(self, gamestate):  
        ...
        if bees_lst:
            if self.turns_to_chew:
                ...
                random_bee = random.choice(bees_lst)
